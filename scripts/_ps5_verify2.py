...
ref=svc.scrape(REF, include_images=False)
print(json.dumps({'queries':build_search_queries(identity_from_price_item(ref))[:8]}, ensure_ascii=False), flush=True)
t0=time.time()
...
# BB: try proxied-only via store aware after pin
print('BB', flush=True)
t0=time.time()
try:
    ...
    print(json.dumps({'ok':False,'secs':round(time.time()-t0,1),'code':getattr(e,'code',type(e).__name__),'msg':str(e)[:200]}, ensure_ascii=False), flush=True)
