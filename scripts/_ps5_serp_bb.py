...
id_kab=identity_from_price_item(kab)
ss=StoreSearchService(fetcher=get_shared_html_fetcher())
print('QUERIES', build_search_queries(id_kab)[:6], flush=True)
for q in build_search_queries(id_kab)[:6]:
  hits=ss.search('shoppingchina', q)
  print(json.dumps({'q':q,'n':len(hits),'top':[{'t':h.title,'u':h.url} for h in hits[:3]]}, ensure_ascii=False), flush=True)
  if hits:
    break
t0=time.time()
bb_hits=ss.search('bestbuy','playstation 5 slim digital')
print(json.dumps({'bb_search':True,'secs':round(time.time()-t0,1),'n':len(bb_hits),'top':[{'t':h.title,'u':h.url} for h in bb_hits[:3]]}, ensure_ascii=False), flush=True)
t0=time.time()
try:
  ...
  print(json.dumps({'bb_scrape':True,'secs':round(time.time()-t0,1),'title':bb.title,'price':str(bb.price),'avail':bb.available,'pid':bb.product_id}, ensure_ascii=False), flush=True)
