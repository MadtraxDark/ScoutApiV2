...
resp=svc.match(MatchRequest(reference_url=REF, stores=['shoppingchina','amazon_us'], persist=False, include_review=True, max_candidates_per_store=5))
print(json.dumps({'matches':[{'store':h.store,'decision':h.decision,'title':h.product.title,'url':h.product.url,'query':h.search_query,'reasons':[r.code for r in h.reasons]} for h in resp.matches],'unmatched':resp.unmatched_stores,'errors':[{'store':e.store,'code':e.code,'message':e.message[:160]} for e in resp.errors]}, ensure_ascii=False, indent=2), flush=True)
